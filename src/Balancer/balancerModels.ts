...
import { BalancerUser, ERC20Token, BalancerUserPoolJoin, BalancerUserPoolSwap, BalancerPool } from "../../generated/schema";
import { createOrLoadBadgeUser, EarnedBadgeEventData } from "../Emblem/emblemModels";
import { incrementProgress } from "../Emblem/metricProgress";
import { BADGE_METRIC_BALANCER_POOLS_JOINED_COUNT, BADGE_METRIC_BALANCER_POOLS_JOINED_COUNT_V1, BADGE_METRIC_BALANCER_SWAPS_COUNT, BADGE_METRIC_BALANCER_SWAPS_COUNT_V1 } from "../Emblem/metrics";
...
export function createOrLoadBalancerUserPoolJoin(
  ...
  let balancerUserPoolJoin = BalancerUserPoolJoin.load(balancerUserPoolJoinId);
  if (balancerUserPoolJoin == null) {
    ...
    incrementProgress(balancerUserId, BADGE_METRIC_BALANCER_POOLS_JOINED_COUNT, eventData);
    incrementProgress(balancerUserId, BADGE_METRIC_BALANCER_POOLS_JOINED_COUNT_V1, eventData);
  }
...
export function createOrLoadBalancerUserPoolSwap(
  ...
  let balancerUserPoolSwap = BalancerUserPoolSwap.load(balancerUserPoolSwapId);
  if (balancerUserPoolSwap == null) {
    ...
    incrementProgress(balancerUserId, BADGE_METRIC_BALANCER_SWAPS_COUNT, eventData);
    incrementProgress(balancerUserId, BADGE_METRIC_BALANCER_SWAPS_COUNT_V1, eventData);
  }
