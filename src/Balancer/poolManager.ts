...
import { createOrLoadBalancerPool, createOrLoadBalancerUserPoolJoin, createOrLoadBalancerUserPoolSwap } from "./balancerModels";
import { incrementProgress } from "../Emblem/metricProgress";
import { BADGE_METRIC_BALANCER_POOLS_CREATED, BADGE_METRIC_BALANCER_POOLS_CREATED_V1, BADGE_METRIC_BALANCER_POOLS_CREATED_V2 } from "../Emblem/metrics";
import { createOrLoadEmblemEntityStats, EarnedBadgeEventData } from "../Emblem/emblemModels";
...
export function processPoolCreated(event: PoolCreated): void {
  ...
  incrementProgress(userAddressString, BADGE_METRIC_BALANCER_POOLS_CREATED_V2, eventData);
}
...
export function processPoolCreatedv1(event: LOG_NEW_POOL): void {
  ...
  incrementProgress(userAddressString, BADGE_METRIC_BALANCER_POOLS_CREATED, eventData);
  incrementProgress(userAddressString, BADGE_METRIC_BALANCER_POOLS_CREATED_V1, eventData);
}
